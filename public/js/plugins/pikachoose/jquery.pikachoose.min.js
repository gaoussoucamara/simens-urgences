...
this.image=$("<img>").appendTo(this.wrap);this.imgNav=$("<div class='pika-imgnav'></div>").insertAfter(this.image);this.imgPlay=$("<a></a>").appendTo(this.imgNav);this.counter=$("<span class='pika-counter'></span>").appendTo(this.imgNav);/**/this.fermer=$("<div class='pika-fermer'> </div>").appendTo(this.imgNav);/**/if(this.options.autoPlay){this.imgPlay.addClass('pause');}else{this.imgPlay.addClass('play');}
...
$(this).css({"height":gap.height,"width":gap.width,"opacity":0.01}).delay(delay).animate({"opacity":1},self.options.animationSpeed,function(){if(self.aniDiv.find(" div").index(this)==24){self.finishAnimating(data);}});});break;case 6:self.aniDiv.height(self.image.height()).hide().css({'background':'url('+data.source+') top left no-repeat'});self.aniDiv.children('div').hide();self.aniDiv.css({width:0}).show().animate({width:self.image.width()},self.options.animationSpeed,function(){self.finishAnimating(data);self.aniDiv.css({'background':'transparent'});});break;case 7:self.wrap.css({overflow:'hidden'});self.aniImg.height(self.image.height()).hide().attr('src',data.source);self.aniDiv.hide();self.image.css({position:'relative'}).animate({left:"-"+self.wrap.outerWidth()+"px"});self.aniImg.show();self.aniWrap.css({left:self.wrap.outerWidth()}).show().animate({left:"0px"},self.options.animationSpeed,function(){self.finishAnimating(data);});break;}},finishAnimating:function(data){this.animating=false;this.image.attr('src',data.source);this.image.attr('alt',data.imageAlt);this.image.attr('title',data.imageTitle);this.image.css({left:"0"});this.image.show();var self=this;$('<img />').on('load',function(){self.aniImg.fadeOut('fast');self.aniDiv.fadeOut('fast');}).attr('src',data.source);var cur=data.index+1;var total=this.thumbs.length; 

this.fermer.html("<script> " +
		           " $('#pika2 .pika-fermer #supp_"+cur+"').click(function(){ /*alert('"+cur+"');*/ supprimerImage('"+cur+"'); }); " +
		           " $('#pika4 .pika-fermer #supp_"+cur+"').click(function(){ /*alert('"+cur+"');*/ supprimerImage('"+cur+"'); }); " +
		           " $('#pika6 .pika-fermer #supp_"+cur+"').click(function(){ /*alert('"+cur+"');*/ supprimerImage('"+cur+"'); }); " +
		           " $('#pika8 .pika-fermer #supp_"+cur+"').click(function(){ /*alert('"+cur+"');*/ supprimerImage('"+cur+"'); }); " +
		           " $('#pika10 .pika-fermer #supp_"+cur+"').click(function(){ /*alert('"+cur+"');*/ supprimerImage('"+cur+"'); }); " +
				 "</script>" +
				 "<img id='supp_"+cur+"' style='width:16px; height:16px; cursor:pointer;' src='../images/images/sup.png' title='supprimer'>"); 

this.counter.html(cur+"/"+total);if(data.clickThrough!==""){if(this.anchor===null){this.anchor=this.image.wrap("<a>").parent();}
...
var e={data:{self:this}};return this.playClick(e);},updateThumbs:function(){var self=this;this.thumbs=this.list.find('img');this.thumbs.each(function(){self.createThumb($(this),self);});}});
