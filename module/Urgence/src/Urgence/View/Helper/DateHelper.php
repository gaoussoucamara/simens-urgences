...
class DateHelper extends AbstractHelper{
	...
	public function convertDateTimeHm ($dateTime) {
		$date = substr($dateTime, 8, 2).'/'.substr($dateTime, 5, 2).'/'.substr($dateTime, 0, 4);
	 ...
	 * @return string
	 */
	public function getDateInDateTimeDHM ($datetime) {
		$date = substr($datetime, 0, 10);
		return $date;
	}
	
	/**
	 * Recuperer la date dans le format datetime avec date, heure et minute et seconde
	 * @param  $datetime
	 * @return string
	 */
	
	public function getTimeInDateTimeDHM ($datetime) {
		$time = substr($datetime, 13, 5).":00";
		return $time;
	}
