...
class AdminController extends AbstractActionController
{
    ...
    public function bienvenueAction() 
    {
    	...
    	else if($user['role'] == "infirmier-tri")
    	{
    		return $this->redirect()->toRoute('urgence', array('action' => 'admission'));
    	}
    	else if($user['role'] == "infirmier-service")
    	{
    		return $this->redirect()->toRoute('urgence', array('action' => 'admission'));
    	}
    	...
    	echo '<a style="font-size: 20px; color: red;" href="'.$this->baseUrl().'public/admin/logout">Terminer</a>';
    	exit();
    }
    ...
    public function modifierPasswordAction()
    {
    	...
    	$this->layout ()->setTemplate ( 'layout/modifierInfosUtilisateur' );
    	...
    	$formUtilisateur->get ( 'service' )->setValueOptions ( array_merge( array(""), $listeService ) );
 		...
 		$request = $this->getRequest();
  		...
  		if ($request->isPost()){
  			...
  			//ENREGISTREMENT DE L'IMAGE DE L'AGENT --- ENREGISTREMENT DE L'IMAGE DE L'AGENT
  			//ENREGISTREMENT DE L'IMAGE DE L'AGENT --- ENREGISTREMENT DE L'IMAGE DE L'AGENT
  			...
  			$anciennePhoto = $this->getPersonnelTable()->getPersonne($user['id_employe'])['PHOTO'];
  			
  			if ($img != false) {
  				if($anciennePhoto){ //SI LA PHOTO EXISTE BIEN ELLE EST SUPPRIMER DU DOSSIER POUR ETRE REMPLACER PAR LA NOUVELLE
  					unlink ( $this->baseUrlRacine().'public/img/photos_personnel/' . $anciennePhoto . '.jpg' );
  				}
  				imagejpeg ( $img, $this->baseUrlRacine().'public/img/photos_personnel/' . $nomPhoto . '.jpg' );
  					
  				//ON ENREGISTRE AVEC LA NOUVELLE PHOTO
  				$this->getUtilisateurTable()->modifierPersonne($donnees, $user['id_employe'], $nomPhoto);
  			} else {
  				//PAS DE NOUVELLE PHOTO
  				$this->getUtilisateurTable()->modifierPersonne($donnees, $user['id_employe'], $anciennePhoto);
  			}
...
//   				return $this->redirect()->toRoute('facturation' , array('action' => 'liste-naissance'));
  		...
  		if(!$user ['Photo']){ $user ['Photo'] = 'identite'; }
